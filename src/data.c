...
// 读取标注文件
box_label *read_lable_file(const char * path, int * box_label_count)
{
    char labelpath[4096];
    replace_image_to_label(path, labelpath);
    int count = 0;
    int i;
    box_label *boxes = read_boxes(labelpath, &count);
    *box_label_count = count;
    return boxes;
}
// 读取标注文件
int read_lable_file_1_1(const char *path,int index, int num_boxes, int truth_size, float *truth, int classes, int flip, float dx, float dy, float sx, float sy,
    int w_new,int h_new,int net_w, int net_h,int *last_count,
    float angle,//旋转角度
    float cut_dx,//剪切后图片偏移量
    float cut_dy,//剪切后图片偏移量
    float cut_w,//剪切后图片宽度
    float cut_h//剪切后图片高度
    )
{
    ...
    float x, y, w, h;
    float centerX = cut_w / 2;
    float centerY = cut_h / 2;

    double radian = angle * 3.1415926535897932384626433832795 / 180.;//角度转换为弧度
    int id;
    int sub = 0;
    
    // 计算裁剪后的标签坐标
    for (i = 0; i < count; ++i) {
        x = (boxes[i].x*sx - cut_dx) / cut_w;
        y = (boxes[i].y*sy - cut_dy) / cut_h;
        w = boxes[i].w*sx / cut_w;
        h = boxes[i].h*sy / cut_h;
        id = boxes[i].id;
        int track_id = boxes[i].track_id;
        truth[(i - sub + index)*truth_size + 0] = x;
        truth[(i - sub + index)*truth_size + 1] = y;
        truth[(i - sub + index)*truth_size + 2] = w;
        truth[(i - sub + index)*truth_size + 3] = h;
        truth[(i - sub + index)*truth_size + 4] = id;
        truth[(i - sub + index)*truth_size + 5] = track_id;
    }
    // 计算旋转角度
    for (i = 0; i < count; ++i) {


        float p1 = truth[(i - sub + index)*truth_size + 0] * cut_w;
        float p2 = truth[(i - sub + index)*truth_size + 1] * cut_h;
        float p3 = truth[(i - sub + index)*truth_size + 2] * cut_w;
        float p4 = truth[(i - sub + index)*truth_size + 3] * cut_h;

        float beg_x = p1 - (p3/2);
        float beg_y = p2 - (p4/2);
        float end_x = beg_x+ p3;
        float end_y = beg_y + p4;
        ...
        float aX1 = (pX1 - centerX)*cos(-radian) - (pY1 - centerY)*sin(-radian) + centerX;
        float aY1 = (pX1 - centerX)*sin(-radian) + (pY1 - centerY)*cos(-radian) + centerY;

        float aX2 = (pX2 - centerX)*cos(-radian) - (pY2 - centerY)*sin(-radian) + centerX;
        float aY2 = (pX2 - centerX)*sin(-radian) + (pY2 - centerY)*cos(-radian) + centerY;

        float aX3 = (pX3 - centerX)*cos(-radian) - (pY3 - centerY)*sin(-radian) + centerX;
        float aY3 = (pX3 - centerX)*sin(-radian) + (pY3 - centerY)*cos(-radian) + centerY;

        float aX4 = (pX4 - centerX)*cos(-radian) - (pY4 - centerY)*sin(-radian) + centerX;
        float aY4 = (pX4 - centerX)*sin(-radian) + (pY4 - centerY)*cos(-radian) + centerY;
        ...
        truth[(i - sub + index)*truth_size + 0] = ((x_min + x_max) / 2) / cut_w;
        truth[(i - sub + index)*truth_size + 1] = ((y_min + y_max) / 2) / cut_h;
        truth[(i - sub + index)*truth_size + 2] = (x_max - x_min) / cut_w;
        truth[(i - sub + index)*truth_size + 3] = (y_max - y_min) / cut_h;
    }
    // 拼图
    for (i = 0; i < count; i++)
    {
        x = truth[(i - sub + index)*truth_size + 0];
        y = truth[(i - sub + index)*truth_size + 1];
        w = truth[(i - sub + index)*truth_size + 2];
        h = truth[(i - sub + index)*truth_size + 3];
        truth[(i - sub + index)*truth_size + 0] = ((x*cut_w)*((w_new) / cut_w) + dx) / (net_w);
        truth[(i - sub + index)*truth_size + 1] = ((y*cut_h)*((h_new) / cut_h) + dy) / (net_h);
        truth[(i - sub + index)*truth_size + 2] = w*cut_w*((w_new) / cut_w) / (net_w);
        truth[(i - sub + index)*truth_size + 3] = h*cut_h*((h_new) / cut_h) / (net_h);
    }
    *last_count = count;
    free(boxes);
    return min_w_h;
...
box_label * truth_to_boxs(float *truth_tmp, int truth_tmp_count,int boxes,int truth_size )
{
    box_label*   boxs = (box_label*)xcalloc((truth_tmp_count + 1), sizeof(box_label)); ;
    int count = 0;
    for (count = 0; count < truth_tmp_count; ++count) {
        float *truth_ptr = truth_tmp + count * truth_size;
        if (!truth_ptr[0]) break;
        ...
        float h = truth_ptr[3];
        int id = truth_ptr[4];
        int track_id = truth_ptr[5];

        //printf(" boxes[count].track_id = %d,  id = %d ,count = %d   \n", track_id, id,count);

      
        boxs[count].track_id = track_id;
        boxs[count].id = id;
        boxs[count].x = x;
        boxs[count].y = y;
        boxs[count].h = h;
        boxs[count].w = w;
        boxs[count].left = x - w / 2;
        boxs[count].right = x + w / 2;
        boxs[count].top = y - h / 2;
        boxs[count].bottom = y + h / 2;
    }
    return boxs;
}
int fill_truth_detection(float *truth_tmp,int truth_tmp_count, int num_boxes, int truth_size, float *truth, int classes, int flip, float dx, float dy, float sx, float sy,
    int net_w, int net_h)
{

    int count = truth_tmp_count;
    int i;
    box_label *boxes = truth_to_boxs(truth_tmp, truth_tmp_count, num_boxes, truth_size);
    int min_w_h = 0;
    float lowest_w = 1.F / net_w;
    float lowest_h = 1.F / net_h;
    randomize_boxes(boxes, count);
...
static int static_augmentation_count = 0;
    ...
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs)
{
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            float *truth_tmp = (float*)xcalloc(truth_size * boxes, sizeof(float));
          
            mat_cv *src = NULL;
            mat_cv *src1 = NULL;
            mat_cv *src_cut_rotate1 = NULL;
            char *filename1;
            int box_index = 0;
            int flag = (c >= 3);
     
           
            static_augmentation_count++;
            int augmentation_count = static_augmentation_count;

            //float angle = augmentation_count % (360 + 8);
            float angle = random_gen() %(45+2);//45度 旋转角度
            angle -= 2;
            if (angle < 0)
                angle = 0;
            ...
            if (true)
            {
                ...
                int w_new1;
                int h_new1;
                int index = random_gen() % (m - 4);
                int box_label_count1;
                filename1 = random_paths[i];
                // 读取图像文件
                src1 = load_image_mat_cv(filename1, flag);
                // 读取标注文件
                box_label * box_labels = read_lable_file(filename1, &box_label_count1);
                // 剪切旋转       
                src_cut_rotate1 = image_data_cut_rotate(src1, box_labels, box_label_count1,angle, &cut_dx1, &cut_dy1,1);

                src = image_data_augmentation_picture_puzzle_1_1(src_cut_rotate1, w_tmp, h_tmp, flip, dhue, dsat, dexp,
                    gaussian_noise, blur, boxes, truth_size, truth_tmp, &w_new1, &h_new1);
                float oh1 = get_height_mat(src1);
                float ow1 = get_width_mat(src1);
                float dx1 = 0;
                float dy1 = 0;
                // 读取标注文件
                int last_count = 0;
                box_index = 0;
                read_lable_file_1_1(filename1, box_index, boxes, truth_size, truth_tmp, classes, 0, dx1, dy1,  ow1, oh1, w_new1, h_new1, w_tmp, h_tmp, &last_count,
                    angle,//旋转角度
                    cut_dx1,//剪切后图片偏移量
                    cut_dy1,//剪切后图片偏移量
                    get_width_mat(src_cut_rotate1),//剪切后图片宽度
                    get_height_mat(src_cut_rotate1)//剪切后图片高度
                    );
                box_index += last_count;
                free(box_labels);
            }

            // 拼图结束
            if (augmentation_count < 200)
            {
                image tmp_ai = mat_to_image_cv(src);
                char buff[1000];
                int t;
                for (t = 0; t < boxes; ++t) {
                    box b = float_to_box_stride(truth_tmp + t * truth_size, 1);
                    //if (!b.x) break;
                    int left = (b.x - b.w / 2.)*(w_tmp);
                    int right = (b.x + b.w / 2.)*(w_tmp);
                    int top = (b.y - b.h / 2.)*(h_tmp);
                    int bot = (b.y + b.h / 2.)*(h_tmp);
                    draw_box_width(tmp_ai, left, top, right, bot, 1, 150, 100, 50); // 3 channels RGB
                }
                sprintf(buff, "img/augmentation_%d_%d_%d_%d", augmentation_count, random_index, i, random_gen());
                save_image(tmp_ai, buff);
                free_image(tmp_ai);
            }
 
            //int index = load_random_image_mat(paths, flag,m);
            //mat_cv *src = random_image_mat[index];
            //char *filename = random_image_filename[index];
            if (src == NULL) {
            ...
            int min_w_h = fill_truth_detection(truth_tmp, box_index, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h);
            ...
            if (augmentation_count < 200)
            {
                ...
                sprintf(buff, "img2/aug_%d_%d_%d_%d", augmentation_count,random_index, i, random_gen());
                int t;
                for (t = 0; t < boxes; ++t) {
            ...
            if (show_imgs && i_mixup == use_mixup)   // delete i_mixup
            {
                ...
                char buff[1000];
                //sprintf(buff, "aug_%d_%d_%s_%d", random_index, i, basecfg((char*)filename), random_gen());
                sprintf(buff, "img/aug_%d_%d_%d", random_index, i, random_gen());
                int t;
                for (t = 0; t < boxes; ++t) {
                    box b = float_to_box_stride(d.y.vals[i] + t*truth_size, 1);
                    if (!b.x) break;
            ...
            release_mat(&src);
            release_mat(&src1);
            release_mat(&src_cut_rotate1);
            //random_image_mat[index] = NULL;
            free(truth);
            free(truth_tmp);
        }
...
void *run_thread_loop(void *ptr)
{
    ...
    while (!custom_atomic_load_int(&flag_exit)) {
        ...
        *args_local = args_swap[i];
        ...
        double timeBegin = what_time_is_it_now();

        load_thread(args_local);

       // printf("读取一组文件⽤时约: %lf毫秒\n", what_time_is_it_now() - timeBegin);
