...
extern "C" mat_cv * image_data_augmentation_picture_puzzle_1_1(mat_cv* mat1, int w, int h,
    int flip,
    float dhue, float dsat, float dexp,
    int gaussian_noise, int blur, int num_boxes, int truth_size, float *truth,int *w_new1, int *h_new1)
{
    
    cv::Mat *mat_ptr = NULL;
    try {
        cv::Mat img1 = *(cv::Mat *)mat1;
        cv::Mat size1;
        if (img1.cols * h / img1.rows >w)
        {
         
            // 缩放图像
            cv::resize(img1, size1, cv::Size(w, img1.rows*w / img1.cols), 0, cv::INTER_LINEAR);
        }
        else
        {
            // 缩放图像
            cv::resize(img1, size1, cv::Size(img1.cols * h / img1.rows, h), 0, 0, cv::INTER_LINEAR);
        }
        ...
        *h_new1 = size1.rows;
        ...
        cv::Size size(w, h);
        cv::Mat img_merge(size, img1.type());
        //2.在新建合并图像中设置感兴趣区域
        cv::Mat outImg1 = img_merge(cv::Rect(0, 0,  size1.cols, size1.rows));
        //3.将待拷贝图像拷贝到感性趣区域中
        size1.copyTo(outImg1);
        mat_ptr = new cv::Mat(img_merge);
        return (mat_cv *)mat_ptr;
    }
    catch (...) {
...
extern "C" mat_cv * image_data_augmentation_picture_puzzle_2_1(mat_cv* mat1, mat_cv* mat2, int w, int h,
    int flip,
    float dhue, float dsat, float dexp,
    int gaussian_noise, int blur, int num_boxes, int truth_size, float *truth, int *w_new1, int *h_new1, int *w_new2, int *h_new2)
{
    cv::Mat *mat_ptr = NULL;
    try {
        cv::Mat img1 = *(cv::Mat *)mat1;
        cv::Mat img2 = *(cv::Mat *)mat2;
        cv::Mat size1;
        cv::Mat size2;

        if (img1.rows*(w / 2) / img1.cols < h)
        {
            // 缩放图像
            cv::resize(img1, size1, cv::Size((w / 2), img1.rows*(w/2) / img1.cols), 0, cv::INTER_LINEAR);
        }
        else
        {
            // 缩放图像
        ...
        if (img2.rows*(w / 2) / img2.cols<h)
        {
            // 缩放图像
            cv::resize(img2, size2, cv::Size((w / 2), img2.rows*(w / 2) / img2.cols), 0, cv::INTER_LINEAR);
        }
        else
        {
            // 缩放图像
            cv::resize(img2, size2, cv::Size(img2.cols * h / img2.rows, h), 0, 0, cv::INTER_LINEAR);
        }
        ...
        *h_new2 = size2.rows;
        ...
        //2.在新建合并图像中设置感兴趣区域
        cv::Mat outImg1 = img_merge(cv::Rect(0, 0, size1.cols, size1.rows));
        cv::Mat outImg2 = img_merge(cv::Rect(w / 2, 0, size2.cols, size2.rows));

        //3.将待拷贝图像拷贝到感性趣区域中
        size1.copyTo(outImg1);
        size2.copyTo(outImg2);
...
extern "C" mat_cv * image_data_augmentation_picture_puzzle_4_1(mat_cv* mat1, mat_cv* mat2, mat_cv* mat3, mat_cv* mat4, int w, int h,
    int flip,
    float dhue, float dsat, float dexp,
    int gaussian_noise, int blur, int num_boxes, int truth_size, float *truth
    , int *w_new1, int *h_new1, int *w_new2, int *h_new2
    , int *w_new3, int *h_new3, int *w_new4, int *h_new4)
{
    cv::Mat *mat_ptr = NULL;
    try {
        cv::Mat img1 = *(cv::Mat *)mat1;
        cv::Mat img2 = *(cv::Mat *)mat2;
        cv::Mat img3 = *(cv::Mat *)mat3;
        cv::Mat img4 = *(cv::Mat *)mat4;
        cv::Mat size1;
        cv::Mat size2;
        cv::Mat size3;
        cv::Mat size4;

        if (img1.rows*(w / 2) / img1.cols < (h/2))
        {
            // 缩放图像
            cv::resize(img1, size1, cv::Size((w / 2), img1.rows*(w / 2) / img1.cols), 0, cv::INTER_LINEAR);
        }
        ...
        {
            // 缩放图像
            cv::resize(img1, size1, cv::Size(img1.cols * (h / 2) / img1.rows, (h / 2)), 0, 0, cv::INTER_LINEAR);
        }


        if (img2.rows*(w / 2) / img2.cols < (h / 2))
        {
            // 缩放图像
            cv::resize(img2, size2, cv::Size((w / 2), img2.rows*(w / 2) / img2.cols), 0, cv::INTER_LINEAR);
        }
        ...
        {
            // 缩放图像
            cv::resize(img2, size2, cv::Size(img2.cols * (h / 2) / img2.rows, (h / 2)), 0, 0, cv::INTER_LINEAR);
        }

        if (img3.rows*(w / 2) / img3.cols < (h / 2))
        {
            // 缩放图像
            cv::resize(img3, size3, cv::Size((w / 2), img3.rows*(w / 2) / img3.cols), 0, cv::INTER_LINEAR);
        }
        else
        {
            // 缩放图像
            cv::resize(img3, size3, cv::Size(img3.cols * (h / 2) / img3.rows, (h / 2)), 0, 0, cv::INTER_LINEAR);
        }


        if (img4.rows*(w / 2) / img4.cols < (h / 2))
        {
            // 缩放图像
            cv::resize(img4, size4, cv::Size((w / 2), img4.rows*(w / 2) / img4.cols), 0, cv::INTER_LINEAR);
        }
        else
        {
            // 缩放图像
            cv::resize(img4, size4, cv::Size(img4.cols * (h / 2) / img4.rows, (h / 2)), 0, 0, cv::INTER_LINEAR);
        }
        ...
        *h_new4 = size4.rows;
        ...
        cv::Mat outImg1 = img_merge(cv::Rect(0, 0, size1.cols, size1.rows));
        cv::Mat outImg2 = img_merge(cv::Rect(w / 2, 0, size2.cols, size2.rows));
        cv::Mat outImg3 = img_merge(cv::Rect(0, h / 2, size3.cols, size3.rows));
        cv::Mat outImg4 = img_merge(cv::Rect(w / 2, h / 2, size4.cols, size4.rows));

        //3.将待拷贝图像拷贝到感性趣区域中
        size1.copyTo(outImg1);
        size2.copyTo(outImg2);
        size3.copyTo(outImg3);
        size4.copyTo(outImg4);
...
// 剪切，旋转
mat_cv * image_data_cut_rotate(mat_cv *img, box_label * box_labels,int box_labels_count,  float angle, float * dx,float * dy,int top_cut)
{
    cv::Mat *mat_ptr = NULL;
    try {
        ...
        float w = 0;
        float h = 0;
        int count = box_labels_count;
        // 设置裁剪区域
        for (int i = 0; i < count; i++)
        {
            if (box_labels[i].left < x)
                x = box_labels[i].left;
            if (box_labels[i].right > w)
                w = box_labels[i].right;
            if (top_cut)
            {
                if (box_labels[i].top < y)
                    y = box_labels[i].top;
                if (box_labels[i].bottom > h)
                    h = box_labels[i].bottom;
            }
            else
            {
                y = 0.01;
        ...
        int new_x = x * mat.cols;
        int new_y = y * mat.rows;
        int new_w = w * mat.cols;
        int new_h = h * mat.rows;


        // 裁剪
        cv::Mat mat_cut = mat(cv::Rect(new_x, new_y, new_w- new_x, new_h- new_y));

        /*
            对旋转的进行改进，由于图形是一个矩形，旋转后的新图像的形状是一个原图像的外接矩形
            因此需要重新计算出旋转后的图形的宽和高
            */
        int width = mat_cut.cols;
        int height = mat_cut.rows;
        double radian = angle * CV_PI / 180.;//角度转换为弧度
        double width_rotate = width;//fabs(width*cos(radian)) + fabs(height*sin(radian));
        double height_rotate = height; //fabs(width*sin(radian)) + fabs(height*cos(radian));
        //旋转中心 原图像中心点
        ...
        cv::Mat imgOut;
        if (mat_cut.channels() == 1)
        {
            cv::warpAffine(mat_cut, imgOut, m1, cv::Size(width_rotate, height_rotate), cv::INTER_LINEAR, 0, cv::Scalar(0));
        }
        else if (mat_cut.channels() == 3)
        {
            cv::warpAffine(mat_cut, imgOut, m1, cv::Size(width_rotate, height_rotate), cv::INTER_LINEAR, 0, cv::Scalar(0, 0, 0));
        }
        mat_ptr = new cv::Mat(imgOut);

        //save_image(mat_to_image(mat_cut), "d:\\a.jpg");
        //save_image(mat_to_image(imgOut), "d:\\b.jpg");
        // 抖动，偏移
        *dx = (imgOut.cols - mat_cut.cols)/2 + new_x;
        *dy = (imgOut.rows - mat_cut.rows)/2 + new_y;
        return (mat_cv *)mat_ptr;
    }
    catch (...) {
        cerr << "OpenCV exception: image_data_cut_rotate \n";
    }
    if (mat_ptr) delete mat_ptr;
    return NULL;
}
