...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, float thresh, float iou_thresh, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        pthread_join(load_thread, 0); // 等待载入数据线程完成
        train = buffer;
        if (net.track) {
        ...
        load_thread = load_data(args); // 创建载入数据线程
      ...
      //  printf("载入一组图像时长: %lf 秒 \n", load_time);
        ...
        if (ngpus == 1) {
            ...
            loss = train_network_waitkey(net, train, wait_key);//等待训练完成
        }
        else {
       ...
       // printf("训练一组文件时时长: %lf 秒\n", what_time_is_it_now() -beginTrainTime);
        ...
        //if (i % 100 == 0) {
        if ((iteration >= (iter_save + 5000) || iteration % 5000 == 0) ||
            (iteration >= (iter_save + 5000) || iteration % 5000 == 0) && net.max_batches < 10000)
        {
            iter_save = iteration;
#ifdef GPU
